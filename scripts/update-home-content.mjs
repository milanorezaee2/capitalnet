 ...
 * ================================================
 * یک‌ پذیری به‌روزرسانی حرفه‌ای متن‌های صفحه Home
  ...
  try {
    for (const update of homeContentUpdates) {
      ...
      if (checkError?.code === 'PGRST116' || !existing) {
        ...
        if (insertError) {
          console.error(`❌ خطا در درج '${update.key}':`, insertError);
        } else {
        ...
        if (updateError) {
          console.error(`❌ خطا در به‌روزرسانی '${update.key}':`, updateError);
        } else {
    ...
    console.log('\n✨ به‌روزرسانی محتوای صفحه Home به‌طور موفق انجام شد!');
  } catch (error) {
